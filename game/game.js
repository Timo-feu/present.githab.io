...
class Class_Time
{
    ...
    NoTime()
    {
        if(this.countHours >= 12){this.Check=false;}
...
class Class_Job 
{
    ...
    jobs = {
        ...
        'Грузчик':['10', 'Очень тяжелая работаю придется переносить тяжелые вещи, но при этом большой заработок для подростка.', ''],
    };
    ...
    Work()
    {
        ...
        if(!Engine.time.NoTime()) {return this;}
        if(Engine.time.countHours-5 >=5) {alert('К сожалению у вас не хватает времени.'); return this;}
    ...
    Delit()
    {
        this.Name = undefined;
        this.Cost = undefined;
        document.querySelector('.job-cont').style.display='none';
        alert('Вы уволились! Можно устроиться на другую работу!')
    }
}
